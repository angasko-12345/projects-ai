...
### A3 — Shared ProcessRuntime (P1) ✅ DONE 2026-09-16

> Delivered: `agentops/runtime.py` (`ProcessRuntime`, `ProcessResult`, `OperationCancelled`, `SpawnFactory`); `AgentRunner` + `VerificationKernel` + legacy `Verifier` delegate to it; kernel no longer imports runner internals; unified Windows/POSIX spawn policy; no-window taskkill; custom-factory direct-kill cleanup; CancelledError termination; thread-free cancel polling; `on_running` lifecycle preserved; duplicate-name validation before persistence; fail-fast failure evidence dominates sibling cancellation. 16 new tests; suite 336 OK; exe rebuilt + archive-inspected + smoke-tested. Copilot review closed (5 fixed, 1 deferred to A8); opencode review adjudicated and closed. Plan: `.agents/plans/a3-runtime-plan.md`. Output: `.agents/outputs/a3-runtime.md`.
...
> Delivered: `FailureEvidence` leaf dataclass + structured-first `classify(evidence=)`; evidence built at agent/verification/legacy recording sites (executable-only agent commands, redacted peeks, scrubbed mappings); additive `structured_evidence` column, schema v7; 19 new tests; suite 356 OK. Copilot review closed (5 fixed); opencode review adjudicated and closed. Plan: `.agents/plans/a4-evidence-plan.md`. Output: `.agents/outputs/a4-evidence.md`.
