...
This is the canonical Pi-specific instruction file. The root `pi_AGENTS.md` is only a compatibility entrypoint for tools that discover it at the repository root.

This file applies when AgentOps selects the `pi` agent or when Pi creates, modifies, or reviews AgentOps. Explicit user instructions, current source code, and current tests take precedence over historical memory.
...
1. Read `.agents/AGENTS.md`, this file, `tasks/task.md`, and `tasks/after-task.md` when present.
...
- Only commands allowlisted in `agents/agents.yaml` may run as agent or verification commands.
...
- A new failure or skip is attributable to the current work until proven otherwise.
...
- Report what changed, commands run, test results, review disposition, and residual risk. Keep the chat summary short; detailed history belongs in `.agents/`.
